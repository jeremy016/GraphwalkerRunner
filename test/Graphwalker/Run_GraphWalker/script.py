...
def v_Landing_GooglePlusPage() :
    ...
    current_url = temp['driver'].current_url
    assert(current_url=='https://plus.google.com/+JustupCloud#google-plus')
...
def v_Landing_FBPage() :
    ...
    current_url = temp['driver'].current_url
    assert(current_url=='https://www.facebook.com/Justupcloud/#facebook')
    return
...
def e_btn_FreeRegister() :
    ...
    temp['driver'].execute_script("window.scrollTo(0, 0);")
...
def e_btn_Login_NoPassword():
    ...
    assert(alertContent==temp['Language']['enter_password'])
...
def e_btn_Login_LoginFail():
    ...
    temp['driver'].find_elements_by_xpath("//div[@id='alertMessage']/button")[0].click()
    print temp['Language']['login_fail']
    print alertContent
...
def e_Login_Footer_ContactUs():
    global temp
    print( "e_Login_Footer_ContactUs" )
